...
import { ArrowRight, BarChart3, Building2, MessageSquareText, Sparkles, Star } from "lucide-react";
    ...
    <main>
          ...
          <div className="space-y-7">
            ...
            <div className="space-y-5">
              ...
              <p className="max-w-2xl text-lg leading-8 text-muted-foreground">
                ReviewPilot gives each company its own rating page, turns structured answers into a useful review draft,
                and keeps every generated review visible for admins.
              </p>
            </div>
          ...
          <div className="rounded-lg border bg-background p-5 shadow-sm">
            <div className="mb-5 flex items-center justify-between border-b pb-4">
              <div>
                ...
                <p className="font-semibold">{demoCompany?.name ?? "Your company"}</p>
              </div>
              <span className="rounded-md bg-amber-100 px-3 py-1 text-sm font-medium text-amber-900">4.8/5</span>
            </div>
            <div className="space-y-4">
              {["Service quality", "Communication", "Recommendation"].map((label, index) => (
                <div key={label} className="rounded-md border p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <span className="text-sm font-medium">{label}</span>
                    <span className="text-sm text-muted-foreground">{5 - (index === 2 ? 1 : 0)}/5</span>
                  </div>
              ...
              <div className="rounded-md bg-accent p-4 text-sm leading-6 text-accent-foreground">
                “The experience felt organized, friendly, and easy from start to finish...”
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="page-shell py-16">
        <div className="mb-8 max-w-2xl">
          <h2 className="text-3xl font-semibold tracking-normal">Built for guided review collection</h2>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {sections.map((section) => (
            <Card key={section.title}>
              ...
              <CardContent className="leading-6 text-muted-foreground">{section.copy}</CardContent>
            </Card>
          ))}
        </div>
      </section>
            ...
            <div key={step} className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  The workflow keeps the review grounded in actual customer feedback.
                </p>
              </div>
            </div>
          ))}
      ...
      <section className="page-shell py-16">
        <div className="grid gap-8 lg:grid-cols-[0.75fr_1fr]">
          <div>
            ...
            <h2 className="text-3xl font-semibold tracking-normal">Ready for Postgres persistence</h2>
            <p className="mt-3 leading-7 text-muted-foreground">
              Drizzle schema, migrations, and pooler-friendly environment configuration are already included. Add your
              transaction pooler URL, run migrations, and the fallback demo data steps aside.
            </p>
          </div>
